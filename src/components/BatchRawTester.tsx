    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-7 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="text-[11px] font-mono font-semibold px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-4">
        ...
        <div className="lg:col-span-7 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-1.5 text-xs">
              <label htmlFor="landingRawInput" className="font-semibold text-slate-700 dark:text-slate-200">
                Input (Name, Phone Number)
              </label>
              <button
                type="button"
                onClick={() => setRawText(SAMPLE_RAW_DATA)}
                className="text-indigo-700 dark:text-indigo-300 hover:underline flex items-center gap-1 text-[11px] cursor-pointer font-medium"
              >
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-slate-600 dark:text-slate-300 mt-2">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
            ...
            {rawText && (
              <button
                type="button"
                onClick={() => setRawText('')}
                className="text-xs text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-slate-100 px-2 py-1 ml-auto cursor-pointer font-medium"
              >
                Clear
              </button>
            )}
        ...
        <div className="lg:col-span-5 flex flex-col">
          <div className="text-xs font-semibold text-slate-700 dark:text-slate-200 mb-1.5 flex items-center justify-between">
            <span>Live Conversion Stream</span>
            <span className="text-[10px] text-slate-600 dark:text-slate-300 font-normal">Real-time Preview</span>
          </div>
            ...
            {parsedRecords.length > 0 ? (
              <div className="space-y-1.5 overflow-y-auto max-h-[220px] pr-1 scrollbar-thin">
                  ...
                  >
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <div className="flex items-center justify-end gap-1 mt-0.5">
                        ...
                        <span className="text-[9px] text-slate-700 dark:text-slate-300 font-medium">
                          {rec.operator}
                        </span>
                      </div>
                    </div>
                ...
                {parsedRecords.length > 6 && (
                  <div className="text-center py-1 text-[10px] text-slate-600 dark:text-slate-300 italic font-medium">
                    + {parsedRecords.length - 6} more records parsed
                  </div>
                )}
              </div>
            ) : (
              <div className="h-full flex flex-col items-center justify-center text-center p-4 text-slate-600 dark:text-slate-300">
                <FileText className="w-8 h-8 text-slate-400 dark:text-slate-600 mb-1.5" />
